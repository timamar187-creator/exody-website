...
export async function mountPersona(config) {
  ...
  function pinScrollAcrossFocus(el) {
    if (!COARSE) return;
    // Enter on the TOUCH — before the browser gets a chance to move anything.
    el.addEventListener('pointerdown', enterTyping);
    el.addEventListener('touchstart', enterTyping, { passive: true });
