...
export async function mountPersona(config) {
   ...
   */
  /*
   * Chasing the scroll with rAF still showed Safari's jump-and-return. The
   * page is FROZEN while a field is focused instead: there is nothing to
   * scroll, so the keyboard opens without moving the view at all.
   *
   * Hoisted because a removed input never fires blur — every path that
   * replaces the slot thaws defensively, or the page would stay stuck.
   */
  let frozenY = 0;
  let frozen = false;
  const freezePage = () => {
    if (frozen || !COARSE) return;
    frozen = true;
    frozenY = window.scrollY;
    const b = document.body;
    b.style.position = 'fixed';
    b.style.top = `${-frozenY}px`;
    b.style.left = '0';
    b.style.right = '0';
    b.style.width = '100%';
  };
  ...
  function pinScrollAcrossFocus(el) {
    if (!COARSE) return;
    // Freeze on the TOUCH, not on focus: Safari decides to scroll as the
    // keyboard opens, which is already too late if we wait for focus.
    el.addEventListener('pointerdown', freezePage);
    el.addEventListener('touchstart', freezePage, { passive: true });
    el.addEventListener('focus', freezePage);
    el.addEventListener('blur', thawPage);
  }
