...
function armDeviceTilt() {
  ...
  // iOS gates deviceorientation behind a permission dialog. The prompt is
  // accepted (the LISA reference asks for it too) — but only ONCE: a denial
  // is remembered so a returning visitor is never asked again.
  const listen = () => addEventListener('deviceorientation', onTilt, { passive: true });
  const REMEMBER = 'ps-tilt';
  try {
    const DOE = window.DeviceOrientationEvent;
    if (DOE && typeof DOE.requestPermission === 'function') {
      let prior = null;
      try { prior = localStorage.getItem(REMEMBER); } catch { /* private mode */ }
      if (prior !== 'denied') {
        DOE.requestPermission()
          .then((r) => {
            try { localStorage.setItem(REMEMBER, r === 'granted' ? 'granted' : 'denied'); } catch { /* ignore */ }
            if (r === 'granted') listen();
          })
          .catch(() => undefined);
      }
    } else listen();
  } catch { /* tilt unavailable — the sway covers it */ }
  setInterval(() => {
...
/* Desktop: the figure sits right, so a centred cue runs into it and the
   word SCROLL disappears against him. Anchor the line to the dialogue's
   column instead (owner, 22.08.26). Phones stay centred — the figure is
   centred there. */
@media (min-width:821px){
  .ps-root:not(.ps-app) .ps-invite.ps-cue{justify-content:flex-start;padding-left:clamp(20px,4vw,64px)}
}
@media (max-width:820px){
  ...
  /* TYPING MODE: solid black, composer at the TOP, figure out of the way.
     The keyboard can never cover a field that sits under the header, so the
     browser never scrolls — which is what killed the jump and the stutter. */
  .ps-root.ps-typing{background:#050505}
  .ps-root.ps-typing .ps-canvas{opacity:0;transition:opacity .18s ease}
  .ps-root.ps-typing::after{content:'';position:absolute;inset:0;background:#050505;z-index:2}
  .ps-root.ps-typing .ps-dialogue{top:calc(56px + env(safe-area-inset-top,0px));bottom:auto;z-index:3;
    max-height:calc(100% - 72px);background:#050505;backdrop-filter:none;-webkit-backdrop-filter:none;
    padding:14px clamp(14px,5vw,26px) 18px}
  .ps-root.ps-typing .ps-current{font-size:clamp(16px,4.4vw,20px)}
  .ps-root.ps-typing .ps-ctrl{display:none!important}
  ...
  .ps-root:not(.ps-app) .ps-canvas{transition:transform .16s cubic-bezier(.22,.61,.36,1);transform-origin:50% 18%}
...
.ps-chip{appearance:none;border:0;border-radius:999px;padding:10px 18px;font:500 14px/1 inherit;cursor:pointer;
  background:var(--ps-chip,#fff);color:var(--ps-chip-ink,var(--ps-ink,#141416));box-shadow:0 1px 2px rgba(0,0,0,.12);
...
/* Same left rail as the dialogue column, so it lines up with the chips
   and the text above it (owner, 22.08.26). */
.ps-restart{left:clamp(20px,4vw,64px);bottom:clamp(16px,3vw,40px)}
...
async function buildAsciiCharacter(mount, opts = {}) {
  // 'paper' inverts the luminance ramp (a dark-on-light scene); 'light' keeps
  // the ramp — bright pixels stay dense — and only swaps the ink to dark, so
  // the FIGURE is drawn on a light app instead of the background (22.08.26).
  const paper = opts.palette === 'paper';
  const darkInk = paper || opts.palette === 'light';
  ...
  if (opts.src) {
     ...
     */
    // Phones sample a coarser grid — the glyphs read larger on a small screen
    // and the per-cell repaint stops eating the frame (owner, 22.08.26).
    const COLS = COARSE ? 122 : 184;
    const ROWS = COARSE ? 70 : 106;
    const CW = 9;
    const CHH = 13;
    ...
    const ink = darkInk ? '#181816' : '#e9e9ef';
    ...
    function place() {
      ...
      out.style.top = COARSE ? '0px' : ((h - out.height * sc) / 2) + 'px';
      if (COARSE) out.dataset.cssHeight = String(out.height * sc);
    }
...
export async function mountPersona(config) {
  ...
  if (mode === 'ascii') {
    const pal = config.character?.palette;
    const paper = pal === 'paper';
    // 'light' keeps the app's own background (the stage is transparent in
    // app mode) and only darkens the text.
    const onLight = paper || pal === 'light';
    root.style.setProperty('--ps-bg', paper ? '#efece3' : '#050505');
    root.style.setProperty('--ps-bg2', paper ? '#faf8f2' : '#0b0b0b');
    root.style.setProperty('--ps-ink', onLight ? '#181816' : '#ececf1');
    // Chips invert against the surface they sit on: black with white text on
    // a light app, quiet dark grey on a dark one.
    root.style.setProperty('--ps-chip-ink', pal === 'light' ? '#f7f7f8' : (paper ? '#181816' : '#ececf1'));
    root.style.setProperty('--ps-paper', paper ? '#faf8f2' : '#0d0d0d');
    root.style.setProperty('--ps-chip', pal === 'light' ? '#141416' : (paper ? '#ffffff' : '#1b1b1b'));
  }
  if (mode !== 'crt') {
    const invite = $('.ps-invite');
    invite.classList.add('ps-cue');
    // The separate SCROLL hint vanished against the black ground wherever
    // it did not land on the figure. One line carries both affordances
    // instead (owner, 22.08.26).
    invite.textContent = COARSE
      ? '[TAP] TO START OR SCROLL'
      : '[CLICK] TO START OR SCROLL';
  }
  try {
  ...
  /** Bumped by every playNode; async work from an older node never paints. */
  let nodeGen = 0;
  ...
  function setSpeaking(v) {
    ...
    // The restart control is the ONLY stop: it becomes ■ while he speaks and
    // returns to ↺ when he is quiet.
    const rs = root.querySelector('.ps-restart');
    if (rs) {
      rs.textContent = v ? '\u25A0' : '\u21BA';
      rs.setAttribute('aria-label', v ? 'Stop speaking' : 'Restart');
    }
  }
  const psLog = (m) => {
  ...
  /**
   * Safari scrolls a focused field into view, and with the hero pinned that
   * threw the visitor into the NEXT section the moment they tapped the ask
   * box (owner, 22.08.26). The page position is pinned across the focus and
   * the keyboard animation that follows it.
   */
  /*
   * TYPING MODE — the only thing that actually works on iOS.
   *
   * Every attempt to let the page keep its normal layout and then correct
   * Safari's scroll-into-view produced the jump, the crawl, or the stutter
   * (owner, 22.08.26: "זה חייב לעבוד פרפקט"). So while a field is focused
   * the page STOPS BEING SCROLLABLE and the composer moves to the TOP of the
   * screen, where the keyboard can never cover it. Safari has nothing to
   * scroll and nothing to fix, so it does nothing — and the screen is solid
   * black, which is also what the phone should show while typing.
   */
  let typing = false;
  let restoreY = 0;
  const enterTyping = () => {
    if (typing || !COARSE) return;
    typing = true;
    restoreY = window.scrollY;
    const de = document.documentElement;
    de.dataset.psTyping = '1';
    de.style.overflow = 'hidden';
    document.body.style.overflow = 'hidden';
    root.classList.add('ps-typing');
  };
  let exitTimer = 0;
  const cancelExit = () => {
  ...
  const scheduleExit = () => {
    cancelExit();
    exitTimer = setTimeout(() => { exitTimer = 0; exitTyping(); }, 180);
  };
  const exitTyping = () => {
    cancelExit();
    if (!typing) return;
    typing = false;
    const de = document.documentElement;
    delete de.dataset.psTyping;
    de.style.overflow = '';
    document.body.style.overflow = '';
    root.classList.remove('ps-typing');
    window.scrollTo(0, restoreY);
  };
  if (COARSE) {
    root.addEventListener('pointerdown', cancelExit, true);
    root.addEventListener('touchstart', cancelExit, { capture: true, passive: true });
  }

  function pinScrollAcrossFocus(el) {
    if (!COARSE) return;
    /*
     * Enter on the TOUCH — before the browser can move anything — and CLAIM
     * THE FOCUS in the same gesture. Moving the field to the top cancels the
     * focus Safari was about to grant, which cost a second tap to open the
     * keyboard (owner, 22.08.26). focus() is only allowed to raise the
     * keyboard inside a user gesture, so it must happen here, synchronously.
     */
    let settleUntil = 0;
    const begin = (e) => {
      // Suppress the browser's own tap handling: the field has just moved to
      // the top, so the default action would re-target whatever is now under
      // the finger and blur us — the keyboard opened and shut instantly
      // (owner, 22.08.26). We claim the focus ourselves instead.
    ...
    el.addEventListener('focus', enterTyping);
    el.addEventListener('blur', () => {
      // A blur inside the settling window is the layout move, not the user
      // leaving — take the focus back instead of tearing typing mode down.
  ...
  function renderAsk(node) {
    ...
    send.className = 'ps-chip ps-send';
    send.type = 'submit';
    // Send is send, always. Silencing EXO belongs to the round control.
    send.textContent = '\u2192';
    send.setAttribute('aria-label', 'Ask');
    form.append(input, send);
    form.addEventListener('submit', (e) => {
      e.preventDefault();
      // Sending ends typing mode: the answer should play with EXO on screen,
      // not against the black typing sheet with him faded out.
      exitTyping();
      ...
      const askGen = nodeGen;
      const finish = (line) => {
        if (askGen !== nodeGen) return;
        void speak(line).then((startVoice) => {
          if (askGen !== nodeGen) return;
          startVoice();
          void typeInto($('.ps-current'), line).then(() => {
            if (askGen === nodeGen) renderAsk(node);
          });
        });
      };
      character.think(700);
  ...
  if (COARSE && !config.app) {
    ...
    /**
     * SELF-CALIBRATING (22.08.26). The portrait's ink does not fill its frame,
     * and hand-tuned constants for "where his head starts" drifted with every
     * change. So the stage MEASURES its own render: one cheap scan of the
     * glyph field gives the real top and span of the figure, and the fit is
     * exact instead of approximately right.
     */
    const ink = { top: 0.17, span: 0.76 };
    const calibrate = () => {
      const field = canvas.querySelector('canvas');
      if (!field || !field.width) return;
      try {
        ...
        for (let y = 0; y < h; y += 3) {
          ...
          for (let x = 0; x < w; x += 3) {
            const i = (y * w + x) * 4;
            if (d[i + 3] > 12 && d[i] + d[i + 1] + d[i + 2] > 90) n += 1;
          }
          if (n > 2) { if (first < 0) first = y; last = y; }
        }
        if (first < 0 || last <= first) return;
        ink.top = first / h;
        ink.span = Math.max(0.4, (last - first) / h);
      } catch { /* tainted or not ready — the defaults are close */ }
    };
    const fit = () => {
      ...
      // SIMPLE: the panel pushes him up, and he shrinks to exactly the room
      // that is left — so he is never cut at the top and never overlapped at
      // the bottom (owner, 22.08.26).
      const HEAD_TOP = 72;  // clears the site header
      // The rendered frame carries empty margins around the portrait — ink
      // measured at 0.15 → 0.97 of the field. Fitting the FRAME therefore
      // left a band of nothing above and below him and made him look tiny
      // (owner, 22.08.26). Fit the INK instead: his head lands at HEAD_TOP
      // and his base lands exactly on the panel.
      const INK_TOP = ink.top;
      const INK_SPAN = ink.span;
      // An untouched dialogue is just padding — it must not push him at all.
      const slot = dlg.querySelector('.ps-slot');
      const live =
        (dlg.querySelector('.ps-current')?.textContent || '').trim().length > 0 ||
        (slot ? slot.childElementCount > 0 : false);
      const eats = live ? panelH : 0;
      const room = Math.max(120, stageH - eats - HEAD_TOP);
      const k = Math.min(1.35, room / (fieldH * INK_SPAN));
      canvas.style.transformOrigin = '50% 0%';
      canvas.style.transform =
        `translateY(${Math.round(HEAD_TOP - fieldH * k * INK_TOP)}px) scale(${k})`;
    };
    try {
    ...
    fit();
    // Calibrate once the first frames have painted, then settle.
    setTimeout(() => { calibrate(); fit(); }, 900);
    setTimeout(() => { calibrate(); fit(); }, 3200);
  }
