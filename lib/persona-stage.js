...
const CSS = `
persona-stage{display:block;position:relative;width:100%;height:100vh}
...
async function buildAsciiCharacter(mount, opts = {}) {
  ...
  if (opts.src) {
    ...
    const paintTexture = (t, level, blink) => {
      tctx.drawImage(front, 0, 0, texCanvas.width, texCanvas.height);
...
export async function mountPersona(config) {
  ...
  function pickSynthVoice() {
    ...
    if (!vs.length) return null; // not loaded yet — retry next line
    synthVoice =
  ...
  if ('speechSynthesis' in window) {
    speechSynthesis.addEventListener?.('voiceschanged', () => { synthVoice = undefined; });
  }
    ...
    if (speakHook) {
      try {
        const prepared = await Promise.race([
          speakHook(text, (v) => character.setLevel(muted ? 0 : v)),
          new Promise((resolve) => setTimeout(() => resolve("timeout"), 9000)),
        ]);
        // ONE voice or SILENCE. The synth stand-in gave the character a
        // second, robotic voice mid-conversation (owner, 19.08.26) — worse
        // than a quiet line, since the text types either way. The page keeps
        // misses rare by pre-fetching and caching the script's audio.
        if (prepared && typeof prepared.start === "function") {
          return () => { if (!muted) prepared.start(); };
        }
        return () => {};
      } catch {
        return () => {};
