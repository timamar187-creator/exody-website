...
function armDeviceTilt() {
  ...
  const onTilt = (e) => {
    ...
    // gamma: left/right tilt; beta: front/back around a ~50° hand-held rest.
    const tx = Math.max(-1, Math.min(1, (e.gamma ?? 0) / 22));
    const ty = Math.max(-1, Math.min(1, ((e.beta ?? 50) - 50) / 26));
  ...
  setInterval(() => {
    ...
    const tx = Math.sin(t * 0.5) * 0.22;
    const ty = Math.cos(t * 0.34) * 0.14;
    for (const g of gazeSinks) { g.tx = tx; g.ty = ty; }
  }, 140);
}
...
@media (max-width:820px){
  .ps-root:not(.ps-app) .ps-dialogue{left:0;right:0;top:auto;bottom:0;width:auto;max-height:62vh;padding:26px 20px calc(52px + env(safe-area-inset-bottom,0px));background:linear-gradient(to top,var(--ps-bg,#0b0b0e) 58%,rgba(11,11,14,.62) 84%,transparent);-webkit-mask-image:none;mask-image:none}
  .ps-root:not(.ps-app) .ps-history{display:none}
  .ps-root:not(.ps-app) .ps-current{font-size:clamp(21px,5.6vw,26px);line-height:1.3;min-height:0}
  .ps-root:not(.ps-app) .ps-chip{padding:12px 18px;font-size:14.5px}
  .ps-root:not(.ps-app) .ps-invite.ps-cue{font-size:14px}
  ...
  .ps-root:not(.ps-app) .ps-canvas{transition:transform .45s cubic-bezier(.22,.61,.36,1)}
  .ps-root:not(.ps-app).ps-has-chips .ps-canvas{transform:scale(.8) translateY(-6vh);transform-origin:50% 22%}
  .ps-root:not(.ps-app).ps-has-chips .ps-dialogue{padding-bottom:calc(64px + env(safe-area-inset-bottom,0px))}
}
