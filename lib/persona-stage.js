...
function armDeviceTilt() {
  ...
  // NEVER prompt. iOS gates deviceorientation behind a permission dialog, and
  // a modal on every visit is worse than the feature is good (owner,
  // 22.08.26). We simply listen: platforms that allow it without a prompt
  // drive the gaze, and iOS silently falls through to the idle sway below.
  try {
  ...
  function pinScrollAcrossFocus(el) {
    ...
    el.addEventListener('focus', () => {
      ...
      const hold = () => {
        ...
        if (++n < 40) requestAnimationFrame(hold);
      };
      hold();
    });
  }
