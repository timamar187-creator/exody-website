...
export async function mountPersona(config) {
  ...
  if (COARSE && !config.app) {
    ...
    const fit = () => {
      ...
      const frac = Math.min(0.74, h / (innerHeight || 1));
      const scale = Math.max(0.5, 1 - frac * 0.82);
      const lift = -Math.min(15, frac * 20);
      canvas.style.transform = `scale(${scale}) translateY(${lift}vh)`;
    };
    try {
