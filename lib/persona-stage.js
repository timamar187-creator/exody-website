...
async function buildAsciiCharacter(mount, opts = {}) {
  ...
  function resize() {
    ...
    // The rendered field carries a few empty rows under the figure's base, so
    // aligning the CANVAS to the floor still left a visible gap — overshoot by
    // that much and he actually stands on the bottom edge.
    out.style.top = COARSE
      ? `${h - out.height * s * 0.94}px`
      : `${(h - out.height * s) / 2}px`;
  }
  addEventListener('resize', resize);
  resize();
...
export async function mountPersona(config) {
  ...
  if (COARSE && !config.app) {
    ...
    const fit = () => {
      ...
      // Fit what is LEFT above the panel, with a floor so he never vanishes.
      const scale = Math.max(0.46, 1 - frac * 0.92);
      canvas.style.transformOrigin = '50% 100%';
      canvas.style.transform = `translateY(${-Math.round(h)}px) scale(${scale})`;
    };
    try {
