...
export async function mountPersona(config) {
   ...
   */
  /*
   * KEYBOARD WITHOUT A JUMP.
   *
   * Freezing the body captured the scroll position AFTER Safari had already
   * scrolled, so the page stuck on the next section (owner, 22.08.26). The
   * fix is to capture the stage's own offset on POINTERDOWN — before any
   * browser scrolling can happen — and hold the page there while the field
   * is focused, lifting the panel above the keyboard with the visual
   * viewport so the input stays visible without the browser needing to
   * scroll anything at all.
   */
  let holdY = 0;
  let holding = false;
  const stageOffsetY = () =>
    Math.max(0, Math.round(root.getBoundingClientRect().top + window.scrollY));
  const liftForKeyboard = () => {
    const vv = window.visualViewport;
    const dlg = $('.ps-dialogue');
    if (!dlg) return;
    if (!holding || !vv) { dlg.style.transform = ''; return; }
    const covered = Math.max(0, window.innerHeight - (vv.height + vv.offsetTop));
    dlg.style.transform = covered > 1 ? `translateY(${-covered}px)` : '';
  };
  const markHold = () => { holdY = stageOffsetY(); };
  const startHold = () => {
    if (holding) return;
    holding = true;
    const tick = () => {
    ...
    tick();
    // While typing the page must show BLACK ONLY — the keyboard shrinks the
    // visual viewport and the next (light) section used to peek in under the
    // hero. The host watches this flag and keeps the hero at full layout
    // height until the field is released (owner, 22.08.26).
    try { document.documentElement.dataset.psTyping = '1'; } catch { /* ignore */ }
  ...
  const stopHold = () => {
    if (!holding) return;
    holding = false;
    try { delete document.documentElement.dataset.psTyping; } catch { /* ignore */ }
  ...
  function pinScrollAcrossFocus(el) {
    if (!COARSE) return;
    // Record the anchor BEFORE the browser can move anything.
    el.addEventListener('pointerdown', markHold);
    el.addEventListener('touchstart', markHold, { passive: true });
