...
@media (max-width:820px){
  ...
  .ps-root:not(.ps-app) .ps-dialogue{left:0;right:0;top:auto;bottom:0;width:auto;max-height:min(64svh,560px);overflow-y:auto;overscroll-behavior:contain;-webkit-overflow-scrolling:touch;padding:clamp(16px,4.4vw,24px) clamp(14px,5vw,26px) calc(clamp(16px,4vw,24px) + env(safe-area-inset-bottom,0px));background:linear-gradient(to top,rgba(5,5,7,.97) 66%,rgba(5,5,7,.74) 89%,transparent);-webkit-mask-image:none;mask-image:none}
...
export async function mountPersona(config) {
  ...
  if (COARSE && !config.app) {
    ...
    const fit = () => {
      ...
      const frac = Math.min(0.74, h / stageH);
